...
/** Compact, optional HUD for the temporal sculpture. */
export function mountSonicMemoryControls(memory: TemporalMemoryController): void {
  ...
  root.innerHTML = `
    <button class="sm-pill" type="button" aria-expanded="false">
    ...
    <section class="sm-panel" hidden>
      ...
      <label>Memory <output data-value="memory"></output><input data-control="memory" type="range" min="1" max="10" step="0.25"></label>
  ...
  const update = (): void => {
    ...
    for (const [name, value] of Object.entries({ memory: settings.memorySeconds, propagation: settings.propagation, gain: settings.gain, warp: settings.warp })) {
      const input = root.querySelector<HTMLInputElement>(`[data-control="${name}"]`)!;
      if (document.activeElement !== input) input.value = String(value);
      root.querySelector<HTMLOutputElement>(`[data-value="${name}"]`)!.value = name === 'memory' ? `${value.toFixed(1)} s` : `${value.toFixed(2)}×`;
    }
